...
Since Cockpit is a very powerful tool, it is often desired to inspect which user performed which operation for auditing purposes. Cockpit operations that change state are logged in the Camunda Platform's [user operation log]({{< ref "/user-guide/process-engine/history/history-configuration.md#user-operation-log" >}}) that is part of the process engine history. The log allows to understand
...
To get a more detailed explanation about the Java and REST API methods to get user operations, check out [this url]({{< ref "/user-guide/process-engine/history/history-configuration.md#accessing-the-user-operation-log" >}}).
