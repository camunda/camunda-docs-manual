...
Query for jobs that fulfill given parameters. This method takes the same message body as the [POST query]({{< ref "/reference/rest/job/post-query.md" >}}) and therefore it is slightly more powerful than the [GET query count]({{< ref "/reference/rest/job/get-query-count.md" >}}).
