...
Camunda Platform is a framework to design, implement, and run business processes and decisions. It has three main components:

* An process and decision engine to run BPMN processes and DMN decisions with a Java API
* A REST API that exposes the engine via HTTP
* The web applications Cockpit, Tasklist, and Admin for monitoring processes, involving humans in processes, and administering Camunda Platform

You can set up Camunda Platform in the way that fits your application landscape best:

* **[Standalone process engine]({{< ref "#standalone-remote-process-engine-server" >}})**: Use Camunda Platform's REST API, plus optionally the web applications. This setup is suitable for developers who work with any programming language that can access a REST API.
* **[Embedded process engine]({{< ref "#embedded-process-engine" >}})**: Embed the Camunda Platform libraries into your Java application and use the Java API, plus optionally the REST API and web applications. This setup is suitable for Java developers who want to use the process engine's full feature set and have maximum control. It integrates with frameworks like Spring, Spring Boot, and Java EE.
* **[Shared process engine]({{< ref "#shared-container-managed-process-engine" >}})**: Run Camunda Platform as a managed service in a Java application server and use the Java API, plus optionally the REST API and web applications. This setup is suitable for Java developers who want to run multiple process applications in one server.
...
* Link to installation guides or similar for the individual setups
* More passive -> active conversion
* reduce filler language
* include latest technologies (e.g. Run, OpenAPI and external task clients)
* "web application architecture" has no connection to any other content on the page
* Move the following paragraphs in this section to the "embedded engine" detail description

* I try to keep the names of the subsections in order to not break links
```


The embeddability requirement is at the heart of many architectural decisions within Camunda Platform. For instance, we work hard to make the process engine component a lightweight component with as little dependencies on [third-party libraries]({{< ref "/introduction/third-party-libraries/_index.md" >}}) as possible. 
Furthermore, the embeddability motivates programming model choices such as the capabilities of the process engine to participate in Spring Managed or JTA [transactions and the threading model]({{< ref "/user-guide/process-engine/transactions-in-processes.md" >}}).

# Camunda Platform Architecture

This section provides an overview of the most common setups for Camunda Platform.
