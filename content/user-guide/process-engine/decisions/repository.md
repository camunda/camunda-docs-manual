...
In addition to the decision definitions, the [decision requirements graph]({{< relref "reference/dmn11/drg/index.md" >}}) (i.e. the `definitions` element in the XML) of a deployed DMN resource is transformed into a *Decision Requirements Definition*. It has, among others, these attributes:
...
Note that the versions of the contains decision definitions can be different from the decision requirements definition if they are also deployed inside other DMN resources or as a single decision inside a DMN resource.
