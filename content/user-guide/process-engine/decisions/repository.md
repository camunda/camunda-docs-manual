...
# Versioning of Decision Requirement Diagrams

In addition to the decision definitions, the [decision requirement diagram]({{< relref "reference/dmn11/drd/index.md" >}}) (i.e. the `definitions` element in the XML) of a deployed DMN resource is transformed into a *Decision Requirement Definition*. It has, among others, these attributes:

- `id`: The unique identifier of the Decision Requirement Definition generated by the
  platform.
- `key`: The definitions `id` attribute from the XML file.
- `name`: The definitions `name` attribute from the XML file.
- `version`: The version of the Decision Requirement Definition generated by the platform.

Note that the decision requirement definition is only created if the DMN resource contains more than one decision. 

## The Decision Requirement Definition Key

The decision requirement definition key is equivalent to the `id` attribute of the `definitions` element in the DMN XML.

```xml
<?xml version="1.0" encoding="UTF-8"?>
<definitions xmlns="http://www.omg.org/spec/DMN/20151101/dmn11.xsd" id="my-drd" name="My DRD" namespace="http://camunda.org/schema/1.0/dmn">
  <!-- ... -->
</definitions>
```

When deploying the above DMN XML file, a decision requirement definition having the following properties is created:
...
## The Decision Requirement Definition Version

When a decision requirement diagram is deployed, it is checked whether a definition with the same key is already deployed.
If not, the decision requirement definition is assigned version `1` for this key. If a decision requirement definition with the same
key already exists, the newly deployed definition will become a new version of
the existing one, increasing its version by one.

Note that the versions of the contains decision definitions can be different from the decision requirement definition if they are also deployed inside other DMN resources or as a single decision inside a DMN resource.

# Querying the Decision Repository

All deployed decision definitions and decision requirement definitions can be queried by the repository service API.
...
## Querying Decision Requirement Definitions

The decision requirement definitions can be queried in a similar way to the decision definitions.

```java
// query for the latest version of a decision requirement definition by key
DecisionRequirementDefinition decisionRequirementDefinition = processEngine
  .getRepositoryService()
  .createDecisionRequirementDefinitionQuery()
  .decisionRequirementDefinitionKey(key)
  .latestVersion()
  .singleResult();
  
// query for all versions of decision requirement definitions by name
List<DecisionRequirementDefinition> decisionRequirementDefinitions = processEngine
  .getRepositoryService()
  .createDecisionDefinitionQuery()
  .decisionRequirementDefinitionName(name)
  .list();  
``` 
...
To query decision requirement definitions, the user needs the `READ` permission on the resource `DECISION_REQUIREMENT_DEFINITION`. The
resource id of the authorization is the key of the decision requirement definition.
