...
The BPM platform on Apache Tomcat 7.x uses the default job executor. The default [job executor]({{< relref "user-guide/process-engine/the-job-executor.md" >}}) uses a ThreadPoolExecutor which manages a thread
