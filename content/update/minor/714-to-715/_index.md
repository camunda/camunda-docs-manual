...
4. For developers: [Exception Handling in Task API](#exception-handling-in-task-api)
5. For administrators and developers: [Update of MySQL JDBC Driver in Camunda Docker Images](#update-of-mysql-jdbc-driver-in-camunda-docker-images)
6. For developers: [Adjustments in Metrics](#adjustments-in-metrics)
7. For developers: [DMN Model API generates DMN 1.3 diagrams](#dmn-model-api-generates-dmn-1-3-diagrams)
8. [Changes in Webapps](#changes-in-webapps)
...
9. For administrators and developers: [PostgreSQL Support Clarification](#postgresql-support-clarification)
10. For administrators and developers: [Changes to Camunda 7 Run Start Script](#changes-to-camunda-platform-run-start-script)
11. For administrators and developers: [Java External Task Client: New Version Handling](#java-external-task-client-new-version-handling)
...
1. Rename the webapp resources folder.

            src/main/resources/plugin-webapp/failed-jobs-plugin 
        --> src/main/resources/plugin-webapp/legacy-failed-jobs-plugin

2. Adjust the Plugin ID and paths in the frontend files. This includes custom API calls, resources and the module name.
