...
menu:
  main:
    name: "Manual Installation: WebSphere 9"
    identifier: "installation-guide-full-was-install-manual"
    parent: "installation-guide-full-was"
    pre: "Install and configure the Full Distribution on IBM WebSphere Application Server 9."
...
* `$PLATFORM_VERSION` denotes the version of Camunda 7 you want to install or already have installed, e.g., `7.15.6-ee`.
* `$WAS_DISTRIBUTION` represents the downloaded Camunda 7 distribution for the IBM WebSphere Application Server, e.g., `camunda-ee-ibm-was-$PLATFORM_VERSION.zip`.
...
Camunda 7 uses one or multiple process engines which must be connected to a datasource.
...
1. Install the database schema to create all required tables and default indices using our [database schema installation guide][db-schema-install].

When you create the tables manually, then you have to configure the engine to **not** create tables at startup by setting the `databaseSchemaUpdate` property to `false` (or, in case you are using Oracle, to `noop`). In WebSphere, this is done in the `bpm-platform.xml`, located in the `$WAS_HOME\modules\camunda-ibm-websphere-ear-$PLATFORM_VERSION.ear\camunda-ibm-websphere-service.jar\META-INF\` folder.
...
This section explains how you can use the WebSphere Integrated Solutions Console to configure a work manager to be used by the Camunda 7 jobexecutor. It is recommended to check the [manual of the application server](http://www-01.ibm.com/software/webservers/appserv/was/library/) for additional details.
...
The following steps are required to deploy Camunda 7.

## Camunda Shared Libraries

The shared libraries include the Camunda engine and some utility JARs. The shared libraries must be visible to both Camunda 7 as well as all process applications.
...
Name it `Camunda`. This name is **mandatory**, except when you modify the `deployment.xml` which is located inside the Camunda 7 EAR accordingly.
...
The Camunda 7 distribution for IBM WebSphere Application Server includes one module in the modules folder:
...
The camunda-ibm-websphere-ear is a Java EE application enterprise archive (EAR) providing Camunda 7 services. It contains an embedded rar module.
This camunda-ibm-websphere-rar module is a JCA Resource Adapter providing the jobexecutor service to Camunda 7.
...
3. Select the `camunda-ibm-websphere-ear-$PLATFORM_VERSION.ear` file from the modules folder of Camunda 7 for IBM WebSphere Application Server distribution. Click Next.
...
The web application archive that contains Camunda Cockpit and Camunda Tasklist resides under `$WAS_DISTRIBUTION/webapps/camunda-webapp-ee-was9-$PLATFORM_VERSION.war` in the IBM WebSphere Application Server distribution archive.
...
4. The first page of the wizard opens. Using the file browser, select the `camunda-webapp-ee-was9-$PLATFORM_VERSION.war` file from the distribution and upload it.
...
The Camunda REST API WAR file resides under `$WAS_DISTRIBUTION/webapps/camunda-engine-rest-$PLATFORM_VERSION-was9.war` in the IBM WebSphere Application Server distribution archive.
...
2. Navigate to the **Applications / Application Types / WebSphere** enterprise applications page.
3. Click the **Install** Button
4. The first page of the wizard opens. Using the file browser, select the `camunda-engine-rest-$PLATFORM_VERSION-was9.war` file from the distribution and upload it.
5. Continue to the next page.
6. Select the **"Fast Path"** on the next page.
7. Step 1. Usually no changes are required.
8. Step 2. Usually no changes are required.
...
In order to activate Camunda Connect functionality for a process engine, a process engine plugin has to be registered in the Camunda 7 configuration as follows:
...
In order to activate Camunda Spin functionality for a process engine, a process engine plugin has to be registered in the Camunda 7 configuration as follows:
...
After installing a Process Application (PA) in your IBM WebSphere Application Server, which **does not** include the Camunda 7 dependencies,
