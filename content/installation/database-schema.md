...
This document will guide you through the installation and update of the Camunda [database schema]({{< ref "/user-guide/process-engine/database/database-schema.md" >}}) used by the process engine.
...
Camunda supports a variety of databases that are documented in the [supported environments]({{< ref "/introduction/supported-environments.md#databases" >}}). 

Camunda supports the following ways of installing the database schema:

* Use the database migration tool [Liquibase](https://www.liquibase.org/) with the provided changelog for a semi-automatic installation and update. Liquibase keeps track of database schema changes. This allows you to focus on *when* changes should be applied rather than also on *which* changes are relevant. Starting with version `7.16.0`, Camunda comes with a curated *changelog file* that Liquibase can consume.
...
READ COMMITED is the required isolation level for database systems to run Camunda with. 
...
You can either install your database schema using Liquibase or using the provided SQL scripts manually. You can switch between those mechanisms when updating your Camunda version at a later stage if desired.
...
To install the database schema required for Camunda, we provide a set of scripts with prepared DDL statements.
...
Updating to a newer Camunda minor version also requires a database schema update. You can reuse all the options available for a [schema installation](#installation) here as well.
...
In case you are just updating to a newer patch level of your Camunda installation, a schema update might not be necessary.
...
1. We highly recommend updating to the latest patch version that is within the bounds of the new minor version you are updating to (`$Y`).
    ...
    _Attention_: This step is only relevant when you are using an enterprise version of Camunda, e.g., `7.16.X` where `X > 0`.
